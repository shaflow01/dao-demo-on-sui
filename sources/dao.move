module dao::dao {
    ...
    public fun distribute_task_rewards(
        core_cap: &CoreCap,
        task: &CommunityTask,
        receiver: address,
        ctx: &mut TxContext,
    ...
    public fun submit_proposal(
        ...
        check_role(member_cap, ctx);
        //2.Pay Proposal Fee
        transfer_coin_to_treasury(treasury, coin, PROPOSAL_FEE, ctx);
        //3.get duration time
    ...
    public fun change_proposal_level(
        core_cap: &CoreCap,
    ...
    public fun vote(
        ...
        //2. transfer dao
        transfer_coin_to_treasury(treasury, coin, votes, ctx);
    ...
    public fun claim_proposal_vote(
        member_cap: &MemberCap,
        proposal: &mut Proposal,
        vote_cap: VoteCap,
        treasury: &mut Treasury<DAO>,
        ...
        check_role(member_cap, ctx);
        assert!(
            &vote_cap.proposal_id == object::borrow_id(proposal),
            EProposalCheck
        );
    ...
    public fun claim_proposal_reward(
        member_cap: &MemberCap,
        proposal: &mut Proposal,
        treasury: &mut Treasury<DAO>,
        ctx: &mut TxContext,
    ...
    public fun distribute_membercap(
        ...
        check_role(core_cap, ctx);
        let total_members = &mut dao.total_members;
        *total_members += 1;
    ...
    public fun distribute_corecap(
        init_core_cap: &InitCoreCap,
        receiver: address,
        ctx: &mut TxContext,
    ) {
        check_role(init_core_cap, ctx);
        let core_cap = CoreCap {
            id: object::new(ctx),
            role_address: receiver,
        };
    ...
    public fun distribute_init_corecap(
        init_core_cap: &InitCoreCap,
        receiver: address,
        ctx: &mut TxContext,
    ) {
    ...
    fun transfer_coin_to_treasury(
        ...
        assert!(
            balance::value<DAO>(&treasury.supply) >= amount,
            EInsufficientTreasurySupply
        );
        balance::join<DAO>(
            &mut treasury.supply,
   balance::split<DAO>(coin::balance_mut(coin), amount),
);
}

fun take_coin_from_treasury(
   treasury: &mut Treasury<DAO>,
   amount: u64,
   ctx: &mut TxContext,
): Coin<DAO> {
...
fun update_proposal_vote(proposal: &mut Proposal, votes: u64, is_support: bool) {
   proposal.lock_balance += votes;
   proposal.support += if is_support { votes } else { 0 };
   proposal.against += if !is_support { votes } else { 0 };
}
...
public fun test() {
   ...
   //6. create proposal
   {
       ...
       submit_proposal(
           &member_cap,
           string::utf8(b"The first community proposal"),
           string::utf8(b"....."),
           1,
           &mut coin,
           &mut treasury,
